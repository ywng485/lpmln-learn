...
int main(int argc, char **argv){
	...
	if(file){
        ...
        string count;
		int ignoreFound = str.find("ignore");
        if (ignoreFound >= 0 && ignoreFound != string::npos) {
          count = "\"ig" + to_string(ignorecount) + "\"";
        } else {
          count = to_string(unsatcount);
        }
				...
				if(splitVec.size() == 1 && !issoft){
					...
					try{
						...
						tempstr += "not unsat(" + count + vars + ")";
						if(weightSwitch)
							tempstr +=  ".\n:~ unsat(" + count + vars +"). [" + splitVecSpace[0] + "@0,"+ count+ oldVars +"]\n";
						else
							tempstr +=  ".\n:~ unsat(" + count + vars +"). [" + to_string((int)(weight*mf)) + "@0,"+ count+ oldVars +"]\n";
					...
					if(splitVecSpace.size() == 1){
						...
						// cout << "366\n";
						tempstr = "unsat(" + count + vars + ") :- " + splitVec[1]+ ".\n" +
										+ ":- " + splitVec[1] + ", not " + "unsat(" + count + vars + ").\n"
										+ ":~ "+ "unsat(" + count + vars + ")." + "["+ weightString +","+ count+ oldVars +"]\n" ;
        ...
        if (issoft && ignoreFound >= 0 && ignoreFound != string::npos) {
          ignorecount ++;
        } else if (issoft){
          unsatcount ++;
        }
        cout<<"% a line (igcount: " << to_string(ignorecount) << ")\n";
		cout<<"% a line (unsatcount: " << to_string(unsatcount) << ")\n";
			}
		}
