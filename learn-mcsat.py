...
#clingo3to4 = 'code/clingo3to4_old'
...
#executeCmd('python code/replaceSharpBySpace.py' + ' ' + tmp_lpmln2cl_file)
executeCmd(clingo3to4 + ' ' + tmp_lpmln2cl_file)
# End: Modify input program with weight placeholders
out = executeCmd('clingo ' + tmp_lpmln2cl_file + '.cl ' + evidence + ' 1')
print out
if 'UNSATISFIABLE' in out:
	print 'Evidence and program not satisfiable. Exit.'
	sys.exit()
numUnsat = countUnsatAtom(getSampleFromText(out), rule_idx)
...
for iter_count in range(max_iteration):
	...
	updateWithWeightPlaceHolders(program, program + str(iter_count) + '.weights', rule_idx)

	# Eliminate positive weights from program
	executeCmd(negConvProgram + ' ' + program + str(iter_count) + '.weights' + ' > ' + tmp_posprog)

	# Executing MC-SAT on the program with positive weight eliminated
	executeCmd(lpmlncompiler + ' ' + tmp_posprog + ' > ' + tmp_aspprog)
	#executeCmd('python code/replaceSharpBySpace.py' + ' ' + tmp_aspprog)
	...
	out = executeCmd('clingo ' + tmp_aspprog + '.cl' + ' 1')
	print out
	...
	for _ in range(max_mcsat_iteration):
		...
		cmd = 'clingo5 ' + SMSample_script +  ' -c s=0 ' + tmp_aspprog + '.cl ' + tmp_sat_const_file + ' 1'
		out = ''
	...
	# Compute new weights
	for idx in weights:
		print 'numUnsat[' + str(idx) + ']', numUnsat[idx]
		print 'sample_count', sample_count
		print 'total_mis', total_mis
		prob_gradient = -numUnsat[idx] + float(total_mis[idx])/float(sample_count)
