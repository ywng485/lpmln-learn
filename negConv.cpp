	...
	if(file){
	    ...
	    try{
			...
			for(str; getline(in, str); ){
				...
				if(str[0] == '%'){
					//ignore comments or negative weights
					cout<<str+"\n";
					continue;
				}
				...
				if(splitVec.size() == 1){
					...
					try{
						...
						//tempstr = "unsat(" + to_string(negcount) + vars + ") :-";

						//tempstr +=  "not " + newStr + ".\n" + newStr + ":-" ;

						//tempstr += "not unsat(" + to_string(negcount) + vars + ")";
						//tempstr +=  ".\n:~ unsat(" + to_string(negcount) + vars +"). [" + to_string(weight) + "@0,"+ to_string(negcount)+ oldVars +"]\n";
						tempstr = "0 " + newStr + ".\n" +
							"neg(" + to_string(negcount) + ",\"ignore\"" + oldVars + ") :- not " + newStr + ".\n" +
							"-" + to_string(floatVal) + " :- not neg(" + to_string(negcount) + ",\"ignore\"" + oldVars + ").\n";
					...
					//Process hard rules of form H.
						...
						string tempstr = "unsat("+to_string(negcount)+vars+") :- not " + str + ".\n" +
											str + " :- not unsat(" + to_string(negcount) +vars+ ").\n" +
											":~ unsat(" + to_string(negcount) +vars+"). [1@1," + to_string(negcount) + oldVars + "]";
								negcount++;
						cout << tempstr + "\n";
					}
					...
					if(splitVecSpace.size() == 1){
						...
						// cout << "366\n";
						tempstr = "0 :- " + splitVec[1]+ ".\n" +
										"neg(" + to_string(negcount) + ",\"ignore\"" + oldVars + ") :- " + splitVec[1] + ".\n" +
										"-" + to_string(weight) + " :- not neg(" + to_string(negcount) + ",\"ignore\"" + oldVars + ").\n";
						//tempstr = "unsat(" + to_string(negcount) + vars + ") :- " + splitVec[1]+ ".\n" +
						//				+ ":- " + splitVec[1] + ", not " + "unsat(" + to_string(negcount) + vars + ").\n"
						//				+ ":~ "+ "unsat(" + to_string(negcount) + vars + ")." + "["+ weightString +","+ to_string(negcount)+ vars+"]\n" ;

						// tempstr = ":~ "+ splitVec[1]+ "." + "["+ weightString +","+ to_string(negcount)+ oldVars +"]\n" ;
					...
					// Hard rules/constraints are ignored.
						...
						if(newStr.empty()){
							...
							if(issoft || (!issoft && mode == 0)){
								...
								// "["+ weightString +","+ to_string(negcount)+ oldVars +"]\n" ;
								// tempstr = ":~ "+ splitVec[1]+ "." + "["+ weightString +","+ to_string(negcount)+ oldVars +"]\n" ;
							}
							else{
								tempstr = str + "\n";
							...
							//tempstr = "unsat(" + to_string(negcount) + vars + ") :-" + splitVec[1] + ",not " + newStr + ".\n" + newStr + ":-" + splitVec[1] + ", " + "not unsat(" + to_string(negcount) + vars + ")" + ".\n"+":~" + "unsat(" + to_string(negcount) + vars +")." + " [" + weightString + ","+ to_string(negcount)+ oldVars +"]\n";
							tempstr = "0 " + newStr + ":-" + splitVec[1] + ".\n";
							tempstr += "neg(" + to_string(negcount) + ",\"ignore\"" + oldVars + ") :- " + splitVec[1] + ", not " + newStr + ".\n";
							tempstr += "-" + to_string(weight) + " :- not neg(" + to_string(negcount) + ",\"ignore\"" + oldVars + ").\n";
						}
